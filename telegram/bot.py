...
async def exit_ticket(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Проверяем наличие активного тикета через API
    try:
        ...
        if response.status_code != 200 or not response.json().get('has_active_chat', False):
            await update.message.reply_text(
        ...
        logger.error(f"Ошибка проверки активного тикета: {e}")
        await update.message.reply_text(
            "Произошла ошибка при проверке тикета. Попробуйте позже."
        )
        return ConversationHandler.END
