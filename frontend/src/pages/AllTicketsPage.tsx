...
interface Ticket {
  ...
  main_category?: string; // Главная категория от AI
  subcategory?: string; // Подкатегория от AI
}
  ...
  const loadTickets = async () => {
    try {
      ...
      const pendingTicketsData: Ticket[] = pendingChats.map((chat: any) => ({
        ...
        main_category: chat.main_category,
                      ...
                      {filteredPendingTickets.map((ticket) => (
                        ...
                        >
                          <div>
                            ...
                            {/* Категории от AI */}
                            {(ticket.main_category || ticket.subcategory) && (
                              <div className="flex flex-wrap gap-2 mb-2">
                                {ticket.main_category && (
                                  <span className="px-2 py-1 bg-custom-blue/10 text-custom-blue text-xs rounded-md border border-custom-blue/30">
                                    {ticket.main_category}
                                  </span>
                                )}
                                {ticket.subcategory && (
